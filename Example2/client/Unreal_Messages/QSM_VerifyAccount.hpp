...
struct VerifyAccount {
    ...
    VerifyAccount(std::string userId, std::string userName, std::string password, std::string connect_info) : userId(userId), userName(userName), password(password), connect_info(connect_info) {}
    ...
    static VerifyAccount deserialize(const std::vector<uint8_t>& buffer) {
        ...
        return VerifyAccount(userId, userName, password, connect_info);
    }
};
#pragma pack(pop)
