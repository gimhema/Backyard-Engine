...
struct MakeAccount {
    ...
    MakeAccount(std::string userId, std::string userName, std::string password) : userId(userId), userName(userName), password(password) {}
    ...
    static MakeAccount deserialize(const std::vector<uint8_t>& buffer) {
        ...
        return MakeAccount(userId, userName, password);
    }
};
#pragma pack(pop)
